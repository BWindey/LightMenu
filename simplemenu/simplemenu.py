...
def menu(options: list[str], title: str = "", body: str = "", start_index: int = 0, printer=print) -> int:
    """
    ...
    :param printer: custom print-function, f.e. with the printer from the Rich library
    ...
    while key not in shortcuts and key != 'enter':
        ...
        show_menu(options, selected_index, title, body, printer)
        key = get_key()
...
def show_menu(options: list[str], selected_index: int, title: str, body: str, printer):
    """
    ...
    :param body: piece of text to display between the title and the menu
    :param printer: custom print-function
    """
    if os.name == 'nt':
        os.system("cls")
